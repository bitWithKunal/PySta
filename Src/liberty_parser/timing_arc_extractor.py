...
class TimingGraphBuilder:
                ...
                for arc in cell.timing_arcs:
                    if hasattr(arc, 'timing_type') and arc.timing_type:
                        ...
                        if 'setup' in arc_type:
                            if hasattr(arc, 'setup_time') and arc.setup_time:
                                setup_time = max(setup_time, arc.setup_time)

                        elif 'hold' in arc_type:
                            if hasattr(arc, 'hold_time') and arc.hold_time:
                                hold_time = max(hold_time, arc.hold_time)

                # Store constraints in cell attributes for later use
                if setup_time > 0 or hold_time > 0:
                    self.dff_constraints[cell_name] = {
                        'setup': setup_time,
                        'hold': hold_time
                    }
                    # Also store in cell attributes
                    cell.attributes['setup_time'] = setup_time
                    cell.attributes['hold_time'] = hold_time
    ...
    def _create_nodes(self):
        ...
        for cell in flat_cells:
            ...
            for port_name, net_name in pins.items():
                ...
                # Create node - using only existing parameters from TimingNode class
                node = TimingNode(
                    name=node_name,
                    node_type=node_type,
                    instance=cell_name,
                ...
                node.clock_pin = is_clock_pin
                node.attributes['pin_name'] = port_name
                node.attributes['net_name'] = net_name
                node.attributes['direction'] = pin.direction if lib_cell and pin else 'unknown'
    ...
    def _create_primary_io_nodes(self, flat_cells: List[Dict]):
        ...
        for net_name in all_nets:
            ...
            if not has_driver:
                ...
                node.clock_pin = False
                node.attributes['pin_name'] = None
                node.attributes['net_name'] = net_name
                node.attributes['direction'] = 'input'
                self.nodes[node_name] = node
            ...
            if node.node_type == NodeType.CELL_OUTPUT:
                ...
                for cell in flat_cells:
                    if cell['name'] == node.instance:
                        for port_name, net_name in cell['pins'].items():
                            pin_name = node.attributes.get('pin_name', '')
                            if port_name == pin_name:
    ...
    def _apply_clock_constraints(self):
        ...
        for node in self.nodes.values():
            if node.clock_pin or (node.attributes.get('pin_name', '').upper() in ['CLK', 'CK']):
                # Try to find which clock this pin belongs to
                assigned = False

                # Check if node net matches any clock source
                net_name = node.attributes.get('net_name', '')
                if net_name:
                    for clock in clocks:
                        if net_name in clock.sources:
    ...
    def validate_graph(self) -> Dict[str, Any]:
        ...
        for net_name, capacitance in self.net_capacitance.items():
            drivers = [n for n in self.get_all_nodes()
                      ...
                      n.attributes.get('direction') == 'output']
            if not drivers:
        ...
        for node in self.get_all_nodes():
            if node.attributes.get('direction') == 'output' and 'net' in node.attributes:
    ...
    def print_summary(self):
        ...
        # Fanout statistics
        if self.net_fanout:
            avg_fanout = sum(self.net_fanout.values()) / len(self.net_fanout) if self.net_fanout else 0
            max_fanout = max(self.net_fanout.values()) if self.net_fanout else 0
            logger.info(f"\nNet statistics:")
